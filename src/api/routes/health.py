    ...
    for attempt in range(max_retries):
        try:
            ...
            try:
                ...
                try:
                    conn = psycopg2.connect(settings.postgres_uri_fallback, connect_timeout=timeout)
                    conn.close()
                    logger.info(
                        f"Database connection successful (fallback): {settings.database_fallback_name}"
                    )
                    return (
                        True,
                        f"Connected to fallback database: {settings.database_fallback_name}",
                    )
                except Exception as fallback_error:
                    logger.error(
                        f"Both primary and fallback database connections failed. Primary: {primary_error}, Fallback: {fallback_error}"
                    )
            ...
            logger.warning(
                f"Database connection attempt {attempt + 1} failed: {e}. Retrying in {delay}s..."
            )
            time.sleep(delay)
...
def check_mlflow_connection():
    ...
    try:
        ...
        return (
            ...
            mlflow_status.get("error"),
            mlflow_status.get("dns_resolution"),
        )
    except Exception as e:
...
async def health_check(detailed: bool = False, response: Response = None):
    ...
    try:
        ...
        # Log with DNS status
        logger.debug(
            f"MLflow health check: {services_status['mlflow']} (CB: {mlflow_status['circuit_breaker_state']}, DNS: {dns_status})"
        )
    except Exception as e:
        services_status["mlflow"] = "unhealthy"
    ...
    redis_status, redis_error = check_redis_connection(
        timeout=2.0
    ...
    logger.info(f"Health check completed: {overall_status}", extra={"services": services_status})
    ...
    if detailed:
        try:
            ...
            response_data["system_info"] = {"cpu_percent": 0.0, "memory_percent": 0.0}
...
async def readiness_check():
    ...
    checks.append(
        {
            "name": "model_30_warmed",
            "passed": warmup["warmed"],
            "critical": settings.model_30_prewarm_enabled,
            "warmup_state": warmup["state"],
            "warmed_at": warmup["warmed_at"],
            "last_error": warmup["last_error"],
            "duration_ms": warmup["duration_ms"],
        }
    )
    if not warmup["warmed"]:
        if warmup["state"] == Model30WarmupState.FAILED.value:
            ready = False
        elif settings.model_30_prewarm_enabled:
            ready = False
...
async def mlflow_health_check():
    ...
    try:
        ...
        if status["circuit_breaker_state"] == "OPEN":
            ...
            return Response(
                content=json.dumps(status), status_code=503, media_type="application/json"
            )
...
async def get_detailed_service_status():
    ...
    try:
        from src.utils.mlflow_config import get_circuit_breaker_status, get_mlflow_status
        ...
        return {
            ...
            "mlflow": {"status": mlflow_status, "circuit_breaker": cb_status},
