...
class HokusaiPipeline(FlowSpec):
    ...
    # Pipeline parameters
    baseline_model_path = Parameter("baseline-model", help="Path to baseline model", default=None)
    ...
    def start(self) -> None:
        ...
        logging.info("Starting Hokusai pipeline")
        logging.info(f"Environment: {self.config.environment}")
        logging.info(f"Dry run: {self.dry_run}")
        logging.info(f"Random seed: {self.config.random_seed}")
    ...
    def load_baseline_model(self) -> None:
        ...
        logging.info(f"Loading baseline model from: {self.baseline_model_path}")
        ...
        if self.dry_run:
            ...
            logging.info("Loaded mock baseline model for dry run")
        else:
    ...
    def integrate_contributed_data(self) -> None:
        ...
        logging.info(f"Loading contributed data from: {self.contributed_data_path}")
        ...
        if self.dry_run:
            ...
            logging.info(f"Created mock contributed data: {len(self.contributed_data)} samples")
            ...
            self.data_manifest = integrator.create_data_manifest(
                self.contributed_data, Path(self.contributed_data_path)
            )

            logging.info(f"Integrated dataset: {len(self.integrated_data)} total samples")
            logging.info(f"Data hash: {self.data_manifest['data_hash'][:16]}...")
            ...
            self.contributed_data = integrator.load_data(
                data_path, run_id=current.run_id, metaflow_run_id=current.run_id
            )
            ...
            # Create data manifest
            self.data_manifest = integrator.create_data_manifest(self.contributed_data, data_path)

            logging.info(
                f"Loaded and validated contributed data: {len(self.contributed_data)} samples"
            )
            logging.info(f"Data hash: {self.data_manifest['data_hash'][:16]}...")
    ...
    def train_new_model(self) -> None:
        ...
        logging.info("Training new model with integrated dataset")
            ...
            if self.dry_run:
                ...
                if "label" in self.integrated_data.columns:
                    ...
                    self.new_model = trainer.train_mock_model(
                        X_train, y_train, model_type="hokusai_integrated_classifier"
                    )
                    ...
                    logging.info(
                        f"Trained mock new model with {training_samples} samples "
                        f"({contributed_samples} contributed)"
                    )
                    logging.info(
                        f"Model metrics: accuracy={self.new_model['metrics']['accuracy']:.4f}"
                    )
                    logging.info(f"MLflow run ID: {model_run_id}")
                ...
                feature_columns = [
                    col for col in self.integrated_data.columns if col not in ["label", "query_id"]
                ]
                X_train, X_test, y_train, y_test = trainer.prepare_training_data(
                    self.integrated_data,
                    target_column="label",
                ...
                logging.info(
                    f"Trained RandomForest model with {training_samples} samples "
                    f"({contributed_samples} contributed)"
                )
                logging.info(
    ...
    def evaluate_on_benchmark(self) -> None:
        ...
        logging.info("Evaluating models on benchmark dataset")
        ...
        with mlflow_run_context(
            ...
            if self.dry_run:
                ...
                # Evaluate new model
                new_results = evaluator.evaluate_model(self.new_model, X_benchmark, y_benchmark)

                # Compare models
                comparison = evaluator.compare_models(
                    baseline_results["metrics"], new_results["metrics"]
                )
                ...
                logging.info(f"Completed mock evaluation in {evaluation_time:.2f} seconds")
                logging.info(f"Baseline model performance: {baseline_results['metrics']}")
                logging.info(f"New model performance: {new_results['metrics']}")
                logging.info(f"Delta score: {delta_score:.4f}")
                ...
                if hasattr(self, "integrated_data") and "label" in self.integrated_data.columns:
                    # Use a subset of integrated data as benchmark
                    # (in real scenario, this would be separate)
                    benchmark_data = self.integrated_data.sample(
                        n=min(1000, len(self.integrated_data)), random_state=self.config.random_seed
                    )

                    feature_columns = [
                        col for col in benchmark_data.columns if col not in ["label", "query_id"]
                    ]
                    X_benchmark = benchmark_data[feature_columns]
                    y_benchmark = benchmark_data["label"]
                    ...
                    logging.info(f"Completed evaluation in {evaluation_time:.2f} seconds")
                    logging.info(f"Baseline model performance: {baseline_results['metrics']}")
                    logging.info(f"New model performance: {new_results['metrics']}")
                    logging.info(f"Delta score: {delta_score:.4f}")
    ...
    @step
    def compare_and_output_delta(self) -> None:
        """Calculate performance delta between models and package results.

        Results are packaged for verifier consumption.
        """
        import json
        from pathlib import Path
        ...
        logging.info("Computing DeltaOne and packaging result for verifier")
        ...
        with mlflow_run_context(
            ...
            contributor_data = {
                ...
                "contributor_weights": (
                    self.new_model.get("integration_metadata", {}).get("contribution_ratio", 0.0)
                ),
            ...
            delta_output = {
                ...
                "evaluation_metadata": {
                    ...
                    "evaluation_time_seconds": (
                        self.evaluation_results.get("evaluation_time_seconds", 0)
                    ),
            ...
            log_step_metrics(
                {
                    ...
                    **{
                        ...
                        for metric, delta in delta_computation["metric_deltas"].items()
                    },
                }
            )
            ...
            logging.info("Generating ZK-compatible output...")
            ...
            if zk_is_valid:
                ...
                logging.info(f"ZK-compatible output saved to: {zk_output_file}")

                # Store ZK output for downstream processing
                self.zk_delta_output = zk_output
            else:
                logging.info(f"Warning: ZK output validation failed: {zk_errors}")
            ...
            try:
                ...
                logging.info(f"Warning: Could not log artifact to MLflow: {e}")
            ...
            logging.info("DeltaOne computation completed successfully")
            logging.info(f"DeltaOne score: {self.delta_one:.4f}")
            logging.info(f"Metrics evaluated: {list(delta_computation['metric_deltas'].keys())}")
            logging.info(f"Contributor samples: {contributor_data['contributed_samples']}")
            logging.info(f"Contribution ratio: {contributor_data['contributor_weights']:.2%}")
            logging.info(f"Delta output saved to: {delta_output_file}")
    ...
    def _compute_model_delta(self, baseline_metrics, new_metrics):
        ...
        for metric in common_metrics:
            try:
                ...
                logging.info(f"Warning: Could not compute delta for metric {metric}: {e}")
                continue
        ...
        return {
            ...
            "improved_metrics": [
                metric for metric, info in metric_deltas.items() if info["improvement"]
            ],
            "degraded_metrics": [
                metric for metric, info in metric_deltas.items() if not info["improvement"]
            ],
        }

    @step
    def generate_attestation_output(self) -> None:
        """Generate attestation-ready output with all results."""
        logging.info("Generating attestation output")
        ...
        logging.info(
            f"Generated attestation output with {len(self.delta_results)} metrics evaluated"
        )
        logging.info(
            f"Evaluation completed in "
            f"{self.evaluation_results['evaluation_time_seconds']:.2f} seconds"
        )
    ...
    def monitor_and_log(self) -> None:
        ...
        logging.info("Logging pipeline results")

        # TODO: Integrate with MLflow for proper tracking
        if not self.dry_run:
            logging.info("MLflow logging not yet implemented")
    ...
    def end(self) -> None:
        ...
        logging.info("Saving pipeline outputs")
        ...
        logging.info(f"Attestation output saved to: {attestation_file}")
