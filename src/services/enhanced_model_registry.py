...
class EnhancedModelRegistry(ModelRegistry):
    ...
    def register_tokenized_model_with_events(
        ...
        if current_value < baseline_value:
            raise ValueError(
                f"Model does not meet baseline requirement: " f"{current_value} < {baseline_value}"
            )
        ...
        try:
            ...
            if event_success:
                logger.info(f"Successfully emitted model_ready_to_deploy event for {model_id}")
                result["event_emitted"] = True
            else:
                logger.warning(f"Failed to emit model_ready_to_deploy event for {model_id}")
    ...
    def register_baseline_with_events(
        ...
        **kwargs,
        ...
        entry = self.register_baseline(
            model=model, model_type=model_type, metadata=metadata, **kwargs
        )
