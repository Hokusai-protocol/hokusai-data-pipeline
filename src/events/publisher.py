...
class Event:
    ...
    def __init__(
        self, event_type: EventType, payload: dict[str, Any], source: str = "hokusai-ml-platform"
    ) -> None:
...
class EventPublisher:
            ...
            if handler.can_handle(event_type):
                try:
                    if handler.handle(event):
                        ...
                        logger.warning(
                            f"Handler {handler.__class__.__name__} failed to process event"
                        )
    ...
    def publish_token_ready(
        self, token_id: str, mlflow_run_id: str, metadata: Optional[dict[str, Any]] = None
    ) -> bool:
