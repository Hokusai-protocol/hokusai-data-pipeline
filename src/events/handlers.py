...
class WebhookHandler(EventHandler):
    ...
    def __init__(
        self, webhook_url: str, headers: Optional[dict[str, str]] = None, timeout: int = 30
    ) -> None:
    ...
    def handle(self, event: Event) -> bool:
        ...
        try:
            response = requests.post(
                self.webhook_url, json=event.to_dict(), headers=self.headers, timeout=self.timeout
            )
...
class PubSubHandler(EventHandler):
    ...
    def __init__(self, topic_name: str, project_id: Optional[str] = None) -> None:
        ...
        try:
            ...
            logger.warning(
                "Google Cloud Pub/Sub not available. Install google-cloud-pubsub to use this handler."
            )
    ...
    def handle(self, event: Event) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Published event {event.event_id} to Pub/Sub with message ID: {message_id}"
            )
            return True
