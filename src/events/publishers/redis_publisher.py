...
class RedisPublisher(AbstractPublisher):
    ...
    def publish(self, message: Dict[str, Any], queue_name: Optional[str] = None) -> bool:
        ...
        if isinstance(raw_timestamp, datetime):
            envelope_timestamp = raw_timestamp
        elif isinstance(raw_timestamp, str):
            envelope_timestamp = datetime.fromisoformat(raw_timestamp.rstrip("Z"))
        else:
            envelope_timestamp = datetime.utcnow()
    ...
    def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "status": "unhealthy",
                "error": str(e),
                "queue_depths": {"main": None, "processing": None, "dlq": None},
            }
...
class RedisPublisherWithCircuitBreaker(RedisPublisher):
    ...
    def health_check(self) -> Dict[str, Any]:
        ...
        health.update(
            {"circuit_breaker_state": cb_stats["state"], "circuit_breaker_stats": cb_stats}
        )
    ...
    def consume_messages(
        self, queue_name: Optional[str] = None, timeout: int = 0, process_callback=None
    ) -> None:
        ...
        while True:
            try:
                ...
                message_data = self.client.brpoplpush(queue_name, processing_queue, timeout=timeout)
