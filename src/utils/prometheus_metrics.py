...
class MLflowMetrics:
    ...
    def __init__(self: "MLflowMetrics") -> None:
    ...
    def _init_metrics(self: "MLflowMetrics") -> None:
        ...
        self.connection_status = Gauge(
            "mlflow_connection_status", "MLflow connection status (1=connected, 0=disconnected)"
        )

        self.connection_attempts = Counter(
            "mlflow_connection_attempts_total", "Total number of MLflow connection attempts"
        )

        self.connection_failures = Counter(
            "mlflow_connection_failures_total", "Total number of MLflow connection failures"
        )

        self.feature_default_applied = Counter(
            "feature_default_applied_total",
            "Feature default substitutions applied during serving normalization",
            ["feature", "reason"],
        )

        # Response time metrics
        self.response_time = Histogram(
            "mlflow_response_time_seconds", "MLflow response time in seconds"
        )

        # Info metrics
        self.info = Info("mlflow_info", "MLflow connection information")

        self._initialized = True

    def update_from_status(self: "MLflowMetrics", status: dict[str, Any]) -> None:
    ...
    def record_feature_default_applied(
        self: "MLflowMetrics",
        feature: str,
        reason: str,
        count: int = 1,
    ) -> None:
        """Record a serving-time feature default substitution."""
        if not PROMETHEUS_AVAILABLE or not self._initialized:
            return

        try:
            self.feature_default_applied.labels(feature=feature, reason=reason).inc(count)
        except Exception as e:
            logger.error(f"Error updating feature default metrics: {e}")

    def get_metrics(self: "MLflowMetrics") -> str:
...
def update_mlflow_metrics(status: dict[str, Any]) -> None:
    """Update MLflow metrics from status dict."""
    mlflow_metrics.update_from_status(status)


def record_feature_default_applied(feature: str, reason: str, count: int = 1) -> None:
    """Record a serving-time feature default substitution."""
    mlflow_metrics.record_feature_default_applied(feature, reason, count)
