...
class ErrorHandler:
    ...
    def handle_error(
        ...
        if self.error_counts[error_key] <= max_retries:
            self.logger.info(f"Retry {self.error_counts[error_key]}/{max_retries} for {error_type}")
            return True
    ...
    def create_user_friendly_message(error: Exception) -> str:
        ...
        error_messages = {
            "TokenNotFoundError": (
                "The specified token does not exist. " "Please check the token ID and try again."
            ),
            "TokenInvalidStatusError": (
                "The token is not in the correct status for this operation."
            ),
            "ModelValidationError": (
                "The model failed validation. " "Please check the model format and requirements."
            ),
            "MetricValidationError": (
                "The metric value is invalid. " "Please check the metric name and value range."
            ),
            "DatabaseConnectionError": (
                "Unable to connect to the database. " "Please check your connection settings."
            ),
            "MLflowError": (
                "An error occurred with MLflow. " "Please check your MLflow configuration."
            ),
            "EventPublishError": (
                "Failed to publish event. " "Please check your event configuration."
            ),
        }
