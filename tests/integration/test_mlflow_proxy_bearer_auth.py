...
class TestMLflowProxyBearerAuth:
    ...
    @patch("httpx.AsyncClient.post")
    async def test_mlflow_proxy_rejects_invalid_bearer_token(self, mock_auth_post, client):
    ...
    async def test_mlflow_proxy_handles_streaming_responses(
        self, mock_auth_post, mock_mlflow_request, client, valid_api_key, mock_auth_service_response
    ):
    ...
    async def test_mlflow_proxy_preserves_request_body(
        ...
        # Request body
        request_body = {"name": "test-experiment", "artifact_location": "s3://bucket/path"}
    ...
    async def test_mlflow_health_check_endpoint(self, mock_get, client):
