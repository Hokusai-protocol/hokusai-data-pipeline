...
class ChaosTestFramework:
    ...
    def wait_for_recovery(
        self, max_wait_time: float, check_interval: float = 5.0
    ) -> tuple[bool, float]:
...
class TestServiceFailureScenarios:
    ...
    def test_redis_connection_chaos(self, client):
        ...
        with (
            ...
            assert (
                unhealthy_responses > 0
            ), "Should have some unhealthy responses due to Redis failures"

            print(
                f"📊 Redis chaos: {healthy_responses} healthy, {unhealthy_responses} unhealthy responses"
            )

    def test_circuit_breaker_cascading_failures(self, client):
        """Test circuit breaker behavior with cascading failures."""
        with (
            patch("src.utils.mlflow_config._circuit_breaker") as mock_circuit_breaker,
            patch("src.utils.mlflow_config.get_mlflow_status") as mock_mlflow_status,
            patch("src.api.routes.health.check_database_connection") as mock_db,
            patch("src.api.routes.health._get_redis") as mock_redis_module,
            patch("src.api.routes.health._get_psycopg2") as mock_pg_module,
            ...
            def recovering_mlflow_status():
                ...
                return {"connected": True, "circuit_breaker_state": real_cb.state, "error": None}
    ...
    def test_message_queue_failure_isolation(self, client):
        ...
        with (
            ...
            # Message queue fails
            mock_publisher.return_value.health_check.side_effect = Exception(
            ...
            assert data["status"] in [
                "healthy",
                "degraded",
            ]  # May be degraded but still operational
...
class TestFailureIsolation:
    ...
    def test_non_critical_service_failure_isolation(self, client):
        ...
        with (
            ...
            mock_mlflow_status.return_value = {"connected": False, "circuit_breaker_state": "OPEN"}
    ...
    def test_timeout_handling_isolation(self, client):
        ...
        with (
            ...
            def slow_mlflow_status():
                ...
                return {"connected": True, "circuit_breaker_state": "CLOSED"}
            ...
            print(
                f"✅ Health check completed in {end_time - start_time:.2f}s despite slow services"
            )
