...
class TestCloudTrailAnalyzer:
    ...
    def test_filter_events_by_time_range(self):
        ...
        now = datetime.utcnow()
        events = [
            {"eventTime": (now - timedelta(days=1)).isoformat() + "Z"},
            {"eventTime": (now - timedelta(days=7)).isoformat() + "Z"},
            {"eventTime": (now - timedelta(days=30)).isoformat() + "Z"},
        ]
...
class TestS3LifecycleAnalyzer:
    ...
    def test_audit_bucket_lifecycles(self, mock_boto_client):
        ...
        mock_s3.get_bucket_lifecycle_configuration.side_effect = [
            ...
            ClientError({'Error': {'Code': 'NoSuchLifecycleConfiguration'}}, 'GetBucketLifecycleConfiguration')
        ]
