...
class TestRedisElastiCacheIntegration:
    ...
    def test_factory_creates_authenticated_redis_url_from_env(self):
        ...
        with patch.dict(
            ...
            with patch(
                "src.events.publishers.redis_publisher.redis.ConnectionPool.from_url"
            ) as mock_pool:
    ...
    def test_factory_creates_unauthenticated_url_for_local_dev(self):
        """Test that local development without Redis config is treated as disabled."""
        with patch.dict(os.environ, {"REDIS_HOST": "localhost", "REDIS_PORT": "6379"}, clear=True):
            ...
            with patch(
                ...
                with patch("src.events.publishers.redis_publisher.redis.Redis") as mock_redis_class:
                    ...
                    with pytest.raises(ValueError, match="Redis not enabled"):
                        create_publisher("redis")
