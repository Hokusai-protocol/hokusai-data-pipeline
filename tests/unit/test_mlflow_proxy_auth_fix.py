...
async def test_proxy_should_forward_auth_headers():
    ...
    with patch("src.api.routes.mlflow_proxy_improved.httpx.AsyncClient") as mock_client_class:
        ...
        assert (
            "authorization" in forwarded_headers
        ), "Authorization header was not forwarded to MLflow!"
...
async def test_api_mlflow_route_should_be_accessible():
    ...
    with patch("src.middleware.auth.APIKeyAuthMiddleware.dispatch") as mock_auth:
        ...
        with patch("src.api.routes.mlflow_proxy_improved.httpx.AsyncClient") as mock_client_class:
            ...
            ), "/api/mlflow route returned 404 - route not mounted!"
            assert response.status_code == 200, f"Expected 200, got {response.status_code}"


def test_current_headers_to_remove_config():
    """Ensure auth headers are not part of the removal list."""
    ...
    # Verify explicitly removed headers are safe transport headers only
    assert '"host"' in code
    assert '"content-length"' in code

    # Auth headers should not be explicitly removed
    headers_block = code.split("headers_to_remove = [", 1)[1].split("]", 1)[0]
    assert '"authorization"' not in headers_block
    assert '"x-api-key"' not in headers_block
