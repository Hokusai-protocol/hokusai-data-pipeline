...
class TestMLflowCircuitBreaker:
    ...
    def test_max_recovery_attempts(self):
        ...
        # Third attempt should require extended timeout
        time.sleep(0.2)
        assert cb.is_open()  # Should still be OPEN due to extended timeout
...
class TestCircuitBreakerIntegration:
    ...
    def test_circuit_breaker_with_mlflow_operations(self, mock_mlflow):
        ...
        assert status2["circuit_breaker_state"] == "CLOSED"
        
        # More failures should open circuit
        get_mlflow_status()  # Second failure
        status3 = get_mlflow_status()  # Third failure - should open circuit
        
        assert status3["connected"] is False
        assert status3["circuit_breaker_state"] == "OPEN"
        assert "Circuit breaker is OPEN" in status3["error"]
        
    @patch('src.utils.mlflow_config.mlflow')
    def test_circuit_breaker_recovery(self, mock_mlflow):
        """Test circuit breaker recovery after failures."""
        from src.utils.mlflow_config import get_mlflow_status, _circuit_breaker
        
        # Force circuit breaker to OPEN state
        _circuit_breaker.state = "OPEN"
        _circuit_breaker.last_failure_time = time.time() - 35  # Past timeout
        ...
        assert status["circuit_breaker_state"] in ["HALF_OPEN", "CLOSED"]
