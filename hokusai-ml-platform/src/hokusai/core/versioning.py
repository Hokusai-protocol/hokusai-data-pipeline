...
class ModelVersionManager:
    ...
    def register_version(
        ...
        if version and auto_increment:
            raise VersioningException("Cannot specify both explicit version and auto_increment")
        ...
        if auto_increment:
            ...
            raise VersioningException("Must specify either version or auto_increment")
        ...
        return self.registry.register_baseline(
            model=model, model_type=model_type, metadata=metadata
        )
    ...
    def get_version_history(self, model_type: str) -> List[ModelRegistryEntry]:
        ...
        # Sort by version
        return sorted(entries, key=lambda e: Version(e.version))
    ...
    def compare_versions(
        self, model_type: str, version1: str, version2: str
        ...
        is_improvement = any(
            metric in metrics_delta and metrics_delta[metric] > 0 for metric in improvement_metrics
        )
    ...
    def get_latest_stable_version(self, model_type: str) -> Optional[ModelRegistryEntry]:
        ...
        stable_entries = [e for e in entries if e.tags.get("stable") == "true"]
    ...
    def deprecate_version(self, model_type: str, version: str) -> bool:
        ...
        try:
            self.registry.client.set_model_version_tag(
                name=model_type, version=target.mlflow_version, key="deprecated", value="true"
            )
            self.registry.client.set_model_version_tag(
                name=model_type,
    ...
    def get_previous_version(
        self, model_type: str, current_version: str
