...
class TestABTestConfig:
    ...
    def test_invalid_traffic_split(self) -> None:
    ...
    def test_multi_variant_test(self) -> None:
...
class TestModelTrafficRouter:
    ...
    def test_create_ab_test(self, router) -> None:
    ...
    def test_route_request_deterministic(self, router) -> None:
    ...
    def test_traffic_distribution(self, router) -> None:
    ...
    def test_stop_ab_test(self, router) -> None:
    ...
    def test_record_metric(self, router) -> None:
    ...
    def test_get_ab_test_results(self, router) -> None:
        ...
        assert (
            results.metrics_comparison["accuracy"]["mean_a"]
            < results.metrics_comparison["accuracy"]["mean_b"]
        )
        assert results.winner is not None  # Should have a winner based on metrics

    def test_auto_stop_expired_test(self, router) -> None:
    ...
    def test_concurrent_ab_tests(self, router) -> None:
        ...
        user_id = "user-123"
        routes = [router.route_request(f"test-{i}", user_id) for i in range(3)]

        # Routes should be deterministic but independent
        assert all(isinstance(route, str) for route in routes)

    def test_gradual_rollout(self, router) -> None:
