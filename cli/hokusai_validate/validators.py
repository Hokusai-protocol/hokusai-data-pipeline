...
from .exceptions import SchemaValidationError, PIIDetectionError
...
class SchemaValidator:
        ...
        try:
            ...
            if 'column_types' in schema:
                ...
                if type_errors:
                    ...
                    result['errors'].append(f"Type validation failed for columns: {list(type_errors.keys())}")
            ...
            if 'value_ranges' in schema:
                ...
                if value_errors:
                    ...
                    result['errors'].append(f"Value validation failed for columns: {list(value_errors.keys())}")
    ...
    def _check_value_ranges(self, data: pd.DataFrame, ranges: Dict[str, Dict]) -> Dict[str, List[str]]:
...
class DataQualityChecker:
    ...
    def check(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        if missing_result['percentage'] > self.max_missing_percentage:
            result['issues'].append(f"High missing value percentage: {missing_result['percentage']:.2%}")
    ...
    def _check_consistency(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        for col in text_columns:
            ...
            has_whitespace = data[col].astype(str).apply(
