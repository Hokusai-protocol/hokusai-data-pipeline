...
Examples:
    ...
    parser.add_argument(
        "--schema", help="Path to custom schema file (default: use built-in schema)"
    )
    ...
    parser.add_argument(
        "--zk-check", action="store_true", help="Perform additional ZK proof readiness checks"
    )
    ...
    try:
        ...
        if args.format == "json":
            ...
            output_text_results(
                input_path, is_valid, errors, args, deterministic_hash, zk_ready, validator
            )
