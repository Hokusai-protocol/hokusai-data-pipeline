...
class EmailSignature(dspy.Signature):
    ...
    tone: Optional[str] = dspy.InputField(desc="Desired tone (formal, casual, etc.)", default="professional")
...
class EmailAssistant(dspy.Module):
    ...
    def forward(self, recipient: str, subject: str, context: str, 
                tone: str = "professional", improve: bool = True):
...
def example_usage():
    ...
    result = assistant.forward(
        ...
        context="Discussed Q4 roadmap, need to confirm timeline for feature X, budget concerns raised",
        tone="professional",
        improve=True
    )
